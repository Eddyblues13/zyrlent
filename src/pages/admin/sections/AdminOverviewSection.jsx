...
import { Users, ShoppingCart, Wallet, Server, TrendingUp, Clock } from 'lucide-react'
...
export default function AdminOverviewSection() {
    ...
    const cards = [
        ...
        { label: 'Revenue', value: `₦${Number(stats?.total_revenue ?? 0).toLocaleString()}`, icon: TrendingUp, color: '#FF3B30' },
        { label: 'Pending Funds', value: stats?.pending_funds ?? 0, icon: Clock, color: '#FF9500' },
    ]
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
                {cards.map(card => (
                    <div key={card.label} className="rounded-2xl border border-[rgba(255,255,255,0.06)] bg-[rgba(15,20,60,0.5)] backdrop-blur-xl p-5 relative overflow-hidden group hover:border-[rgba(255,149,0,0.2)] transition">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="rounded-2xl border border-[rgba(255,255,255,0.06)] bg-[rgba(15,20,60,0.5)] backdrop-blur-xl p-6">
                    <h3 className="text-sm font-bold text-white/60 mb-4">Recent Orders</h3>
                    <div className="space-y-3">
                        ...
                        {(stats?.recent_orders ?? []).map(order => (
                            <div key={order.id} className="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
                                ...
                                <span className={`text-xs px-2 py-1 rounded-lg font-medium ${order.status === 'completed' ? 'bg-green-500/10 text-green-400' : order.status === 'pending' ? 'bg-yellow-500/10 text-yellow-400' : 'bg-red-500/10 text-red-400'}`}>
                                    {order.status}
                                </span>
                            </div>
                        ))}
                ...
                {/* Recent Users */}
                <div className="rounded-2xl border border-[rgba(255,255,255,0.06)] bg-[rgba(15,20,60,0.5)] backdrop-blur-xl p-6">
                    <h3 className="text-sm font-bold text-white/60 mb-4">Recent Users</h3>
                    <div className="space-y-3">
                        ...
                        {(stats?.recent_users ?? []).map(u => (
                            <div key={u.id} className="flex items-center gap-3 py-2 border-b border-white/5 last:border-0">
                                ...
                                <span className="text-xs text-white/20">{new Date(u.created_at).toLocaleDateString()}</span>
                            </div>
                        ))}
                    </div>
                </div>
